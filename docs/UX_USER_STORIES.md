...
- システムプロンプトのプリセット（「統合会議アシスタント」）が提供される
...
- **使用タイミング**: Zoom/Teamsなどのオンライン会議中、会議の文字起こしとリアルタイム要約が必要な場合、会議中にAIに質問や意見を求めたい場合
...
- 会話履歴が常に表示される
- システムプロンプトのプリセット（「思考パートナー・タスク管理」）が提供される
- AI にタスクを整理・優先順位付けしてもらえる（ソクラテス的質問法で思考を促進）
...
- **推奨入力モード**: `always_on`
- **推奨オーディオソース**: `mic`
- **使用タイミング**: 作業中にアイデアが浮かんだ時、タスクを音声で記録し思考を深めたい時、問題解決の思考プロセスを促進したい時
...
- システムオーディオキャプチャ状態が表示される
- カスタマイズシステムプロンプト（「統合会議アシスタント」）が提供される
- 会話履歴は最小化できる（必要時のみ表示）

**システムプロンプトテンプレート:**

- **テンプレート ID**: `comprehensive_meeting_assistant`（UC1と共通）
- **推奨入力モード**: `push_to_talk`
- **推奨オーディオソース**: `mixed`（システムオーディオ + マイク）
- **使用タイミング**: Zoom/Teamsなどのオンライン会議中、会議中にAIに質問や意見を求めたい場合
...
- システムプロンプトのプリセット（「リアルタイム翻訳アシスタント」）が提供される
- 会話履歴で翻訳前後の対比が表示される（「原文: [原文] → 翻訳: [翻訳]」形式）
- 言語選択を簡単に切り替えられる
...
- **使用タイミング**: 外国語の会話を理解したい時、多言語会議で翻訳が必要な時、リアルタイムで翻訳を確認したい時
...
- システムプロンプトのカスタマイズ機能が提供される（`default`テンプレートをベースにカスタマイズ）
...
- セッション時間が表示される
- システムプロンプトの簡単な切り替え UI が提供される（利用可能なテンプレートから選択可能）
...
- **テンプレート ID**: 動的変更（タスクに応じて適切なテンプレートを選択）
  - `comprehensive_meeting_assistant`（会議関連タスク）
  - `thinking_task_assistant`（思考・タスク管理）
  - `translation_assistant`（翻訳タスク）
  - `default`（汎用タスク）
...
4. **システムプロンプトのプリセット実装**
   - 「統合会議アシスタント」（`comprehensive_meeting_assistant` - UC1, UC3）
   - 「思考パートナー・タスク管理」（`thinking_task_assistant` - UC2）
   - 「リアルタイム翻訳アシスタント」（`translation_assistant` - UC5）
   - 「デフォルト（汎用アシスタント）」（`default` - UC4, UC6, UC7）
...
3. **システムプロンプトのプリセット** - 主要ユースケースをサポート
   - `comprehensive_meeting_assistant`（UC1, UC3: 統合会議アシスタント）
   - `thinking_task_assistant`（UC2: 思考パートナー・タスク管理）
   - `translation_assistant`（UC5: リアルタイム翻訳アシスタント）
   - `default`（UC4, UC6, UC7: 汎用アシスタント）
